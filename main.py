...
def get_blocked_users(db):
    return db.query(User).filter(User.blocked == True).all()


def get_block_reason_stats(db):
    blocked_users = get_blocked_users(db)
    total = len(blocked_users)
    if total == 0:
        return "Aucune donnée de blocage."

    counts = {}
    for user in blocked_users:
        reason = user.block_reason or "unknown"
    ...
    for reason, count in sorted(counts.items(), key=lambda item: item[1], reverse=True):
        ...
        lines.append(f"- {label} : {count} ({percent}%)")
...
async def admin_callback(update, context, db, user, data):
    ...
    if data == "admin:stats":
        ...
        unfinished = len(get_unfinished_users(db))
        ...
        reasons = get_block_reason_stats(db)
        ...
        text = (
            ...
            f"📣 Broadcasts envoyés : {broadcast_count}\n"
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if is_admin(tg_user.id) and user.state.startswith("admin_waiting_broadcast:"):
            ...
            if target == "all":
                users = [
                    type("Target", (), {"telegram_id": app.telegram_id})()
                    for app in db.query(Application).filter(Application.status == "accepted").all()
                ]
            elif target == "unfinished":
                users = get_unfinished_users(db)
            elif target == "blocked":
                users = get_blocked_users(db)
            else:
                users = [
                    type("Target", (), {"telegram_id": app.telegram_id})()
                    for app in (
            ...
            for target_user in users:
                if target_user.telegram_id in seen:
                    continue

                seen.add(target_user.telegram_id)

                result = await safe_send_message(context.bot, target_user.telegram_id, text)
                if result:
                    sent += 1
                else:
                    failed += 1
